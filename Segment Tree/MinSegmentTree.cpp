...
struct segtree{
    ...
    void set(int i, int v, int x, int lx, int rx){
        ...
        mins[x] = min(mins[2*x+1],mins[2*x+2]); //Sum of Left and Right SubTree
    }
    ...
    // }
    int minSeg(int l, int r, int x, int lx, int rx){
        if(lx>=r or l>=rx){
            return INT_MAX; //No intersection
        }
        if(lx>=l and rx<=r) return mins[x];
        int m = (lx+rx)/2;
        int s1 = minSeg(l,r,2*x+1,lx,m);
        int s2 = minSeg(l,r,2*x+2,m,rx);
        return min(s1,s2);
    }
